...
Future<void> main() async {
  ...
  test('Fetching pull request description', () async {
    final pullrequestDescription = await github.pullrequestBody();
    expect(
  ...
  test('Listing files for PR', () async {
    final files = await github.listFilesForPR(Directory.current);
    expect(files, [
      GitFile(
  ...
  test('Find comment', () async {
    final commentId = await github.findCommentId(user: 'auto-submit[bot]');
