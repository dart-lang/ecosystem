...
void main() {
  group('ApiUseCollector - packages', () {
    var targetPackage = PackageInfo.from({
      'name': 'path',
      'latest': {'version': '0.1.2', 'pubspec': <String, dynamic>{}},
    });
    ...
    test('libraries references', () async {
      ...
      check(
        apiUsageCollector.referringPackages.sortedLibraryReferences,
      ).containsKey('package:path/path.dart');
      check(
        apiUsageCollector.referringLibraries.sortedLibraryReferences,
      ).containsKey('package:path/path.dart');
    });
    ...
    test('class references', () async {
      ...
      check(
        apiUsageCollector.referringPackages.sortedClassReferences,
      ).containsKey('PosixStyle');
      check(
        apiUsageCollector.referringLibraries.sortedClassReferences,
      ).containsKey('PosixStyle');
    ...
    test('extension references', () async {
      ...
      check(
        apiUsageCollector.referringPackages.sortedExtensionReferences,
      ).containsKey('IterableExtension');
      check(
        apiUsageCollector.referringLibraries.sortedExtensionReferences,
      ).containsKey('IterableExtension');
    });
    ...
    test('top-level symbol references', () async {
      ...
      check(
        apiUsageCollector.referringLibraries.sortedTopLevelReferences,
      ).containsKey('join');
